...
import { ImagePlus, MapPin, Tag, Wheat, X } from "lucide-react";
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (post) => {
      queryClient.setQueryData(["community", "feed"], (old: any) => {
        ...
        return {
          ...old,
          pages: [{ ...first, items: [post, ...first.items] }, ...old.pages.slice(1)],
        };
      });
  ...
  const handleSubmit = async () => {
    ...
    if (file) {
      ...
      try {
        ...
        const uploaded = { url: fileUrl, type: mediaType, key, contentType: file.type || "application/octet-stream" };
        mediaItems = [uploaded];
            ...
            <div className="flex flex-wrap gap-2">
              {TAG_OPTIONS.map((tag) => (
                <Button
                  ...
                  className={tags.includes(tag) ? "bg-emerald-600 hover:bg-emerald-700 h-9 px-4" : "h-9 px-4"}
