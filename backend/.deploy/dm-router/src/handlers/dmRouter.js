...
exports.handler = async (event) => {
  try {
    ...
    if (path.startsWith('/dm/conversations/') && path.endsWith('/messages')) {
      ...
      const rawConversationId = parts[3];
      const conversationId = rawConversationId ? decodeURIComponent(rawConversationId) : '';
    ...
    if (method === 'GET' && path.startsWith('/dm/contact-status/')) {
      const rawOtherUid = path.split('/')[3];
      const otherUid = rawOtherUid ? decodeURIComponent(rawOtherUid) : '';
    ...
    if (method === 'DELETE' && path.startsWith('/dm/blocks/')) {
      const rawBlockedUid = path.split('/')[3];
      const blockedUid = rawBlockedUid ? decodeURIComponent(rawBlockedUid) : '';
